...
void UPitchDetectorComponent::BeginPlay()
{
    ...
    // ★ TargetSubmix는 BP에서 하드 레퍼런스로 할당되어 있어야 함
    if (!TargetSubmix)
    {
        UE_LOG(LogTemp, Error, TEXT("[PitchDetector] TargetSubmix is NULL. Assign it in BP."));
        return;
    }

    FAudioDeviceHandle AD = GetWorld() ? GetWorld()->GetAudioDevice() : FAudioDeviceHandle();
    if (AD)
    {
        UE_LOG(LogTemp, Warning, TEXT("[PitchDetector] Device OK, Submix=%s"),
            *GetNameSafe(TargetSubmix));

        AD->RegisterSubmixBufferListener(this, TargetSubmix);
        // bIsDetecting = true; (필요하다면 상태 업데이트)
    }
    else
    {
        UE_LOG(LogTemp, Error, TEXT("[PitchDetector] AudioDevice invalid at BeginPlay"));
    }
...
void UPitchDetectorComponent::OnNewSubmixBuffer(const USoundSubmix* OwningSubmix, float* AudioData, int32 NumSamples, int32 NumChannels, const int32 SampleRate, double AudioClock)
{
    ...
    if (FFTComponent->PerformForwardFFT(WindowedBuffer, FFTOutput))
    {
        ...
        if (Frequency >= MIN_FREQUENCY && Frequency <= MAX_FREQUENCY && Confidence > Threshold)
        {
            ...
            const FPitchInfo Copy = CurrentPitchInfo;
            AsyncTask(ENamedThreads::GameThread, [this, Copy]()
            {
                    OnPitchDetected.Broadcast(Copy);
            });
        }
    }
}
...
{
    if (!TargetSubmix) return;

    if (UWorld* W = GetWorld())
    {
        if (FAudioDeviceHandle AD = W->GetAudioDevice())
        {
            UE_LOG(LogTemp, Warning, TEXT("[PitchDetector] Unregister %s"),
                *GetNameSafe(TargetSubmix));
            AD->UnregisterSubmixBufferListener(this, TargetSubmix);
        }
    }
}
